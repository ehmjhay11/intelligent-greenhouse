...
function SensorData() {
  ...
  const fetchSensors = async () => {
    try {
      ...
      if (result.success) {
        setSensors(result.data.filter((sensor: Sensor) => sensor.isActive));
  ...
  const formatDeviceLabel = (name: string): string => {
    if (!name) return '';
  ...
  if (loading) {
    return (
      <>
        <Head>
          <title>Live Sensor Data - Greenhouse Dashboard</title>
        </Head>
  ...
  const deviceGroups = sensors.reduce((acc: Record<string, Sensor[]>, sensor) => {
      ...
      <main className="page-container" role="main">
        ...
        {sensors.length === 0 ? (
          <div className="text-center p-16 bg-primary-50 rounded-xl border-2 border-dashed border-primary-200 max-w-2xl mx-auto">
            ...
            <button 
              onClick={() => router.push('/manage-sensors')}
