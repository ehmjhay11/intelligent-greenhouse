...
const API = {
  historical: (device: string, range: string) => `${getApiBase()}/sensors/historical?device=${device}&range=${range}`,
  logs: (type: string, from: string, to: string) => `${getApiBase()}/system/logs?type=${type || ''}&from=${from || ''}&to=${to || ''}`,
};

function useHistorical(device: string, range: string, metric: string) {
    ...
    return {
      data: {
        ...
        datasets: filtered.map(s => ({
          label: s.label,
          data: data.map(d => d[s.key as keyof HistoricalDataPoint] as number),
          borderColor: s.color,
      ...
      options: {
        ...
        scales: {
          ...
          y1: { type: 'linear' as const, position: 'right' as const, grid: { drawOnChartArea: false }, title: { display: true, text: 'Percentage (%)' } },
          y2: { type: 'linear' as const, position: 'right' as const, display: false },
        },
      },
    };
  }, [data, metric]);
...
export default function Reports() {
  ...
  const handleExportPDF = async () => {
    ...
    autoTable(doc, { 
      startY: 60, 
      head: [['Timestamp','Temp (°C)','Humidity (%)','Soil Moisture (%)','Light']], 
      body: rows 
    });
    try {
      if (chartRef.current) {
  ...
  return (
    <>
      <Head>
        <title>Reports - Greenhouse Dashboard</title>
      </Head>
